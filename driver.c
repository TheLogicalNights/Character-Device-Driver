...
#define  DEVICE_NAME "Swapnil_Driver_1"

// The device class -- this is a character device driver
#define  CLASS_NAME  "Swapnil_Driver"        
...
{
	printk(KERN_INFO "Swapnil : Driver loaded successfully\n");
	...
	if (majorNumber<0)
	{
		printk(KERN_ALERT "Swapnil : failed to register a major number\n");

		return majorNumber;
	}

	printk(KERN_INFO "Swapnil : registered correctly with major number %d\n", majorNumber);
	...
	printk(KERN_INFO "Swapnil : device class registered correctly\n");
	...
	printk(KERN_INFO "Swapnil : device class created correctly\n");
...
static void __exit char_exit(void)
{
	...
	unregister_chrdev(majorNumber, DEVICE_NAME);
	printk(KERN_INFO "Swapnil : Goodbye from our driver!\n");
}
...
static int dev_open(struct inode *inodep, struct file *filep)
{
	...
	printk(KERN_INFO "Swapnil :  Device has been opened %d time(s)\n", numberOpens);
...
static ssize_t dev_read(struct file *filep, char *buffer, size_t len, loff_t *offset)
{
	...
	error_count = raw_copy_to_user(buffer, message,len);

	if (error_count==0)
	{            // if true then have success
		printk(KERN_INFO "Swapnil :  Sent %d characters to the user\n", size_of_message);
		
		return (size_of_message=0);  // clear the position to the start and return 0
	}
	else 
	{
		printk(KERN_INFO "Swapnil :  Failed to send %d characters to the user\n", error_count);
...
{
	raw_copy_from_user(message,buffer,len);
	//sprintf(message, "%s(%d letters)", buffer, len);   // appending received string with its length

	size_of_message = strlen(message);                 // store the length of the stored message

	printk(KERN_INFO "Swapnil :  Received %d characters from the user\n",size_of_message);
...
{
	printk(KERN_INFO "Swapnil :  Device successfully closed\n");
